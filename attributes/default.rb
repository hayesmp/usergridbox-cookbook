node.override['maven']['3']['version'] = '3.0.5'
node.override['maven']['3']['url'] = 'http://apache.mirrors.tds.net/maven/maven-3/3.0.5/binaries/apache-maven-3.0.5-bin.tar.gz'
node.override['thrift']['configure_options'] = ['--without-ruby']
node.override['java']['oracle']['accept_oracle_download_terms'] = true
node.override['java']['jdk_version'] = '7'
node.override['java']['install_flavor'] = 'oracle'
node.override['cassandra']['user'] = 'cassandra'
node.override['cassandra']['conf_dir'] = "#{Chef::Config[:file_cache_path]}/dsc-cassandra-1.1.11/conf"
node.override['cassandra']['installation_dir'] = "#{Chef::Config[:file_cache_path]}/dsc-cassandra-1.1.11/"
node.override['cassandra']['version'] = '1.11.11'
